window.slideMetadata = [
    ...
    },
    {
        "slide":  42,
        "name":  "Slide36",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 1: S → L := E | Caso general:\rUna proposición de asignación tiene un lado izquierdo L y una expresión derecha E. La acción semántica mira si L es una variable simple o una matriz. | Caso 1: L es una variable simple\rSi x := E, se genera una asignación normal: x := E.lugar. | Caso 2: L es una matriz\rSi A[i] := E, se genera una asignación indexada usando L.lugar como base de la matriz y L.desplazamiento como posición calculada. Si el lado izquierdo es simple, se emite una asignación normal. Si es una matriz, primero se usa el desplazamiento calculado y luego se emite una asignación indexada."
    },
    {
        "slide":  43,
        "name":  "Slide37",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Reglas 2 y 3 | Regla 2: E → E1 + E2\rSi una expresión es una suma, se crea un temporal nuevo para guardar el resultado. Ejemplo: x + y genera t1 := x + y, por lo tanto E.lugar = t1. | Regla 3: E → (E1)\rLos paréntesis no generan código nuevo; solo conservan el lugar de la expresión interna. Si x + y quedó en t1, entonces (x + y) también queda en t1. Una suma genera un temporal nuevo, mientras que los paréntesis solo agrupan la expresión y no generan instrucciones nuevas."
    },
    {
        "slide":  44,
        "name":  "Slide38",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 4: E → L | Caso general:\rUna expresión puede ser un lugar de memoria. Si L es una variable simple, se usa directamente. Si L es una matriz, hay que leer el valor almacenado en la posición calculada. | Caso 1: variable simple\rSi E → x, entonces E.lugar = x. | Caso 2: acceso a matriz\rSi E → A[i], se calcula el desplazamiento y luego se genera algo como t1 := A[t0], donde t0 es la posición calculada y t1 guarda el valor leído. Si L es una matriz, se genera código para leer el contenido de la posición calculada y guardarlo en un temporal."
    },
    {
        "slide":  45,
        "name":  "Slide39",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 5: L → listaE ] | Caso general:\rEsta regla se aplica cuando termina la referencia a la matriz. En ese momento ya tenemos la lista de índices y podemos calcular el desplazamiento. | L.lugar\rRepresenta la base o componente constante de la dirección de la matriz. | L.desplazamiento\rRepresenta la parte variable calculada a partir de los índices. Si cada elemento ocupa 4 bytes, un caso como A[y,z] se parece a (y * cantidad_columnas + z) * 4. Con esto se prepara la dirección que se usará para leer o escribir el elemento."
    },
    {
        "slide":  46,
        "name":  "Slide40",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 6: L → id | Caso general:\rEsta regla indica que L puede ser simplemente una variable. | Atributos\rL.lugar = id.lugar\rL.desplazamiento = null | Interpretación\rEl valor null indica que no se trata de una matriz. Si L es solo un identificador, el compilador usa una asignación normal y no una indexada. Ese null le avisa al compilador que no debe tratarlo como matriz."
    },
    {
        "slide":  47,
        "name":  "Slide41",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 7: listaE → listaE1 , E | Caso general:\rEsta regla se usa cuando ya había una lista de índices y aparece otro índice separado por coma. Así se actualiza el cálculo del desplazamiento acumulado. | Ejemplo: A[y,z]\rPrimero se tiene y. Luego, al encontrar z, el compilador calcula algo como t1 := y * columnas; t1 := t1 + z. Ahora listaE.lugar guarda el cálculo acumulado. | Idea matemática\rPara A[y,z], el desplazamiento lógico es y * número_de_columnas + z. En más dimensiones, cada índice nuevo multiplica lo acumulado por el tamaño de la dimensión siguiente y luego suma el nuevo índice."
    },
    {
        "slide":  48,
        "name":  "Slide42",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 8: listaE → id [ E | Caso general:\rEsta regla reconoce el comienzo de una referencia a matriz. Aquí ya se sabe cuál es la matriz y cuál es su primer índice. | Atributos iniciales\rlistaE.matriz = A\rlistaE.lugar = y\rlistaE.ndim = 1 | Qué significa cada atributo\rlistaE.matriz guarda cuál es la matriz.\rlistaE.lugar guarda el índice actual o el cálculo acumulado.\rlistaE.ndim indica cuántas dimensiones ya fueron procesadas. Esta regla inicia el procesamiento de una matriz y deja preparada la información para seguir acumulando el desplazamiento si aparecen más índices."
    },
    {
        "slide":  49,
        "name":  "Slide44",
