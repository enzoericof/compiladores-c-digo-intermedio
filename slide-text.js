window.slideMetadata = [
    ...
    },
    {
        "slide":  42,
        "name":  "Slide36",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 1: S → L := E | Caso general: | Una proposición de asignación tiene un lado izquierdo L y una expresión derecha E. La acción semántica mira si L es una variable simple o una matriz. | Caso 1: L es una variable simple | Si x := E, se genera una asignación normal: x := E.lugar. | Caso 2: L es una matriz | Si A[i] := E, se genera una asignación indexada usando L.lugar como base de la matriz y L.desplazamiento como posición calculada. Si el lado izquierdo es simple, se emite una asignación normal. Si es una matriz, primero se usa el desplazamiento calculado y luego se emite una asignación indexada."
    },
    {
        "slide":  43,
        "name":  "Slide37",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Reglas 2 y 3 | Regla 2: E → E1 + E2 | Si una expresión es una suma, se crea un temporal nuevo para guardar el resultado. Ejemplo: x + y genera t1 := x + y, por lo tanto E.lugar = t1. | Regla 3: E → (E1) | Los paréntesis no generan código nuevo; solo conservan el lugar de la expresión interna. Si x + y quedó en t1, entonces (x + y) también queda en t1. Una suma genera un temporal nuevo, mientras que los paréntesis solo agrupan la expresión y no generan instrucciones nuevas."
    },
    {
        "slide":  44,
        "name":  "Slide38",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 4: E → L | Caso general: | Una expresión puede ser un lugar de memoria. Si L es una variable simple, se usa directamente. Si L es una matriz, hay que leer el valor almacenado en la posición calculada. | Caso 1: variable simple | Si E → x, entonces E.lugar = x. | Caso 2: acceso a matriz | Si E → A[i], se calcula el desplazamiento y luego se genera algo como t1 := A[t0], donde t0 es la posición calculada y t1 guarda el valor leído. Si L es una matriz, se genera código para leer el contenido de la posición calculada y guardarlo en un temporal."
    },
    {
        "slide":  45,
        "name":  "Slide39",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 5: L → listaE ] | Caso general: | Esta regla se aplica cuando termina la referencia a la matriz. En ese momento ya tenemos la lista de índices y podemos calcular el desplazamiento. | L.lugar | Representa la base o componente constante de la dirección de la matriz. | L.desplazamiento | Representa la parte variable calculada a partir de los índices. Si cada elemento ocupa 4 bytes, un caso como A[y,z] se parece a (y * cantidad_columnas + z) * 4. Con esto se prepara la dirección que se usará para leer o escribir el elemento."
    },
    {
        "slide":  46,
        "name":  "Slide40",
        "text":  "Esquema de traducción para acceder a elementos de matrices | Regla 6: L → id | Caso general: | Esta regla indica que L puede ser simplemente una variable. | Atributos | L.lugar = id.lugar\rL.desplazamiento = null | Interpretación | El valor null indica que no se trata de una matriz. Si L es solo un identificador, el compilador usa una asignación normal y no una indexada. Ese null le avisa al compilador que no debe tratarlo como matriz."
    },
    {
        "slide":  47,
        "name":  "Slide41",
